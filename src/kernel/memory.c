...
/* 得到虚拟地址vaddr对应的pte指针，即返回能够访问该pte的虚拟地址*/
uint32_t *pte_ptr(uint32_t vaddr)
{
    /* 先通过最后一个页表项访问到页表自己 + 再用页目录项pde(页目录内页表的索引)做为pte的索引访问到页表 + 再用pte的索引做为页内偏移*/
    uint32_t *pte = (uint32_t *)(0xffc00000 + ((vaddr & 0xffc00000) >> 10) + PTE_IDX(vaddr) * 4);
    return pte;
}

/* 得到虚拟地址vaddr对应的pde的指针，即返回能够访问该pde的虚拟地址 */
...
/* 在用户空间中申请 pg_cnt * 4kB 个内存,并返回其虚拟地址 */
